<template>
  <MinistryPage
    name="청년부"
    subtitle="자카르타 한인 직장인 및 학생 청년들의 공동체"
    bg-color="from-purple-600 to-purple-500"
    accent-color="bg-purple-500"
    image="https://images.unsplash.com/photo-1547434019-d330eff46f59?w=800&q=80"
    description="저희 한마음교회 청년부는 인도네시아 자카르타 및 근교지역에 거주하는 직장인 및 학생들로 구성되어 있습니다. 조국을 떠나왔던 다니엘이 하나님을 섬기고, 이상을 품으며 민첩하게 살았던 것을 본받아 힘든 해외 생활 속에서 찬양과 기도와 말씀으로 주님의 자녀된 삶을 살고자 노력하며 서로를 격려하고 세워주는 사랑의 공동체입니다."
    :details="details"
  />
</template>

<script setup lang="ts">
import MinistryPage from '@/components/MinistryPage.vue'

const details = [
  { label: '대상', items: ['자카르타 및 근교 거주 청년', '직장인 및 대학생', '20대-30대'] },
  { label: '사역 가치', items: ['찬양과 기도와 말씀', '다니엘의 신앙 본받기', '서로 격려하고 세워주는 공동체', '해외생활 속 믿음의 삶'] },
]
</script>
